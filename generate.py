...
(function () {
  ...
  var audioEls = document.querySelectorAll('[data-speak], [data-speak-all], [data-tts-speed]');
  if (!synth) { audioEls.forEach(function (b) { b.remove(); }); return; }

  var rate = 0.95;            // velocità "Normale"
  var voice = null;
  function pickVoice() {
  ...
  function buildSpeedControls() {
    document.querySelectorAll('[data-tts-speed]').forEach(function (h) {
      h.innerHTML = '<span>Velocità</span><span class="seg">'
        + '<button type="button" data-rate="0.95" class="active">Normale</button>'
